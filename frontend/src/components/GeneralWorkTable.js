  ...
  return (
    <div className="space-y-4" data-testid="general-work-table">
      ...
      {sections.map((section) => {
        ...
        return (
          <div key={section.key} data-testid={`section-${section.key}`} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
            <button
                    ...
                    {section.jobs.map((job) => (
                      <tr
                        ...
                        className={`border-b border-slate-100 dark:border-slate-800/60 last:border-0 cursor-pointer transition-colors ${
                          selected.has(job.id) ? "bg-indigo-50/60 dark:bg-indigo-950/20" : "hover:bg-slate-50/60 dark:hover:bg-slate-800/30"
                        }`}
                      >
                        ...
                        <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-1">
                            <button data-testid="mark-completed-button" onClick={() => complete(job)} title="Sposta in Completati" className="h-8 w-8 rounded-md flex items-center justify-center text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-950/40 transition-colors">
                            ...
                            <button data-testid="delete-work-button" onClick={() => remove(job)} title="Elimina" className="h-8 w-8 rounded-md flex items-center justify-center text-red-500 hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors">
